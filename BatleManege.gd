...
# --- VARIÁVEIS DE ATAQUE ---
var player_cards_that_attacked_this_turn = [] # Lista de cartas que já atacaram
var player_is_attacking = false # Trava para evitar bugs de animação
...
@onready var player_discard = $"../PlayerDiscard"
@onready var enemy_discard = $"../EnemyDiscard"
...
func start_player_turn():
	...
	# Reseta os ataques do jogador
	player_cards_that_attacked_this_turn = []

func start_enemy_turn():
	print("--- Turno do INIMIGO ---")
	
	# Aguarda um pequeno intervalo antes de começar as ações do inimigo
	await get_tree().create_timer(1.0).timeout
	
	# 1. Jogar uma carta do inimigo
	try_play_card()
	await get_tree().create_timer(1.0).timeout
	
	# 2. Fase de ataque do inimigo
	enemy_attack_phase()

func enemy_card_selected(enemy_card):
	# Pega o monstro que VOCÊ selecionou no CardManager
	var attacker = card_manager.selected_monster
	
	if attacker == null:
		return # Não tem ninguém selecionado para atacar
	
	if player_is_attacking:
		return # Já está atacando, aguarde
	
	# Realiza o ataque
	player_is_attacking = true
	
	# Chama a função de combate
	await attack_card(attacker, enemy_card)
	
	# Registra que a carta atacou
	player_cards_that_attacked_this_turn.append(attacker)
	
	# Limpa a seleção e destrava
	if card_manager.selected_monster != null:
		# Move visualmente de volta para a posição original
		card_manager.selected_monster.global_position.y += 20
...
func attack_card(attacker, defender):
	...
	if "health" in defender and "attack" in attacker:
		...
		if defender.has_method("update_visuals"):
			defender.update_visuals()
	
	if "health" in attacker and "attack" in defender:
		attacker.health -= defender.attack
		if attacker.has_method("update_visuals"):
			attacker.update_visuals()
